...
export class AdminService {
  ...
  async create(
    ...
    const role =
      ...
      (await this.roleRepository.findOneBy({ id: createAdminDto.roleId }));
   ...
   if (image) {
    const uploadedImage = await this.uploadService.uploadAdminImage(image, admin.id);
    admin.image = uploadedImage;
    try {
        await this.adminRepository.save(admin);
    } catch (e) {
        await this.uploadService.deleteFile(uploadedImage); // cleanup
        throw e;
    }
}
