...
export class CacheService {
  ...
  private buildKey(domain: string, key: string, config?: CacheConfig): string {
    const prefix = config?.prefix || this.defaultConfigs[domain]?.prefix || domain;
    return `${prefix}:${key}`;
  }
  ...
  async get<T>(domain: string, key: string, config?: CacheConfig): Promise<T | null> {
    try {
  ...
  async del(domain: string, ...keys: string[]): Promise<number> {
    try {
      ...
      this.logger.error(`Cache delete failed for ${domain}:${keys.join(',')}`, error);
      return 0;
    }
  }
  ...
  async exists(domain: string, key: string): Promise<boolean> {
    try {
      ...
      this.logger.error(`Cache exists check failed for ${domain}:${key}`, error);
      return false;
    }
  }
  ...
  async getByPattern<T>(domain: string, pattern: string): Promise<Record<string, T>> {
    try {
  ...
  async deleteByPattern(domain: string, pattern: string): Promise<number> {
    try {
      ...
      this.logger.error(`Pattern delete failed for ${domain}:${pattern}`, error);
      return 0;
    }
  }
  ...
  // Cache statistics
  async getStats(domain?: string): Promise<CacheStats | Record<string, CacheStats>> {
    if (domain) {
  ...
  async mget<T>(domain: string, keys: string[]): Promise<Record<string, T | null>> {
    const cacheKeys = keys.map(key => this.buildKey(domain, key));
  ...
  async mset<T>(domain: string, data: Record<string, T>, config?: CacheConfig): Promise<boolean> {
    try {
      const ttl = config?.ttl || this.defaultConfigs[domain]?.ttl || 3600;
      const pipeline = this.redisService.getClient().pipeline();
